...
export default function ShirtProducts() {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    axios.get("/api/shirt").then((res) => {
              ...
              {products.map((product, index) => (
                ...
                >
                  <tr>
                    ...
                    <td class="px-6 py-4 truncate max-w-xs">{product.title}</td>
                    ...
                    <td class=" gap-4 px-2 py-2 font-medium">
                      <Link
                        href={"/shirtProducts/delete/" + product._id}

import BannerPage3 from "@/components/BannerPage3";
import axios from "axios";
import { useRouter } from "next/router";
import { useEffect, useState } from "react";

export default function EditBanner3() {
  ...
  useEffect(() => {
    if (!id) {
      ...
      axios.get("/api/banner3?id=" + id).then((response) => {
  ...
  return (
    <>
      ...
      <div className="my-10">
        {productInfo && <BannerPage3 {...productInfo} />}
      </div>
    </>
  );
}

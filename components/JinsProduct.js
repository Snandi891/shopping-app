...
const JinsProduct = ({
  ...
  async function createJins(ev) {
    ...
    if (_id) {
      await axios.put("/api/jins", { ...data, _id });
      toast.success(" Product Sucessfully updated!");
    } else {
      await axios.post("/api/jins", data);
  ...
  return (
    <>
      <form onSubmit={createJins} className="mx-auto max-w-screen-sm ">

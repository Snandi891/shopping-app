...
const ShoesProduct = ({
  ...
  async function createShoes(ev) {
    ...
    if (_id) {
      await axios.put("/api/shoes", { ...data, _id });
      toast.success(" Product Sucessfully updated!");
    } else {
      await axios.post("/api/shoes", data);
  ...
  return (
    <>
      <form onSubmit={createShoes} className="mx-auto max-w-screen-sm ">

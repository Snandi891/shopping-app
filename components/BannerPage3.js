...
const BannerPage3 = ({
  ...
  async function createBanner3(ev) {
    ev.preventDefault();
    ...
    if (_id) {
      await axios.put("/api/banner3", { ...data, _id });
      toast.success(" Product Sucessfully updated!");
    } else {
      await axios.post("/api/banner3", data);
  ...
  return (
    <>
      <form onSubmit={createBanner3} className="mx-auto max-w-screen-sm ">

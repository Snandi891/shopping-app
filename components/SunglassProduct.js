...
const SunglassProduct = ({
  ...
  async function createSunglass(ev) {
    ev.preventDefault();
    ...
    if (_id) {
      await axios.put("/api/sunglass", { ...data, _id });
      toast.success(" Product Sucessfully updated!");
    } else {
      await axios.post("/api/sunglass", data);
  ...
  return (
    <>
      <form onSubmit={createSunglass} className="mx-auto max-w-screen-sm ">

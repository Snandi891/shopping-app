...
const BannerPage4 = ({
  ...
  async function createBanner4(ev) {
    ev.preventDefault();
    ...
    if (_id) {
      await axios.put("/api/banner4", { ...data, _id });
      toast.success(" Product Sucessfully updated!");
    } else {
      await axios.post("/api/banner4", data);
  ...
  return (
    <>
      <form onSubmit={createBanner4} className="mx-auto max-w-screen-sm ">

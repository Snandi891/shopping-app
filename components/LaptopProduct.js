...
const LaptopProduct = ({
  ...
  async function createLaptop(ev) {
    ev.preventDefault();
    ...
    if (_id) {
      await axios.put("/api/laptop", { ...data, _id });
      toast.success(" Product Sucessfully updated!");
    } else {
      await axios.post("/api/laptop", data);
  ...
  return (
    <>
      <form onSubmit={createLaptop} className="mx-auto max-w-screen-sm ">
